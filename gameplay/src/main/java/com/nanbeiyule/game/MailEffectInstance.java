...
final class MailEffectInstance {
    ...
    MailEffectInstance(OriginalLobbyEffectAssets.Loaded loaded, MailEffectSpec spec) {
        this.spec = spec;
        runtime = loaded.runtime();
        pageBitmaps = new LinkedHashMap<>(loaded.pages());
    }

    MailEffectSpec spec() { return spec; }
    Spine37Runtime runtime() { return runtime; }
    Integer texture(String pageName) { return textures.get(pageName); }
    ...
    void uploadTextures() {
        for (Map.Entry<String, Bitmap> entry : pageBitmaps.entrySet()) {
            ...
            if (ids[0] == 0) {
                ...
                throw new IllegalStateException("Unable to allocate mail effect texture");
            }
    ...
    void releaseTextures() {
        if (!textures.isEmpty()) {
            int[] ids = textures.values().stream().mapToInt(Integer::intValue).toArray();
